...
def get_parser():
    ...
    parser.add_argument('--data_training_config', default='train_config.json')
    parser.add_argument('--data_validation_config', default='val_config.json')
...
class BambooTrainer():
    ...
    def validate_one_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        for data_pt in list(self.val_config.keys()):
            ...
            steps, start, end, data_size = self.prepare_data(val_data, self.args.val_batch_size)
            data_size_all += data_size

            for step in range(steps):
                # val_data['charge'] = torch.zeros_like(val_data['atom_types'])
                # val_data['total_charge'] = torch.zeros_like(val_data['energy'])
                # val_data['dipole'] = torch.zeros_like(val_data['cell'])
                # val_data['quadrupole'] = torch.zeros(len(val_data['energy']), 5)
        ...
        for k in val_rmse.keys():
            ...
            val_mae[k] = sum(val_mae[k]) / data_size_all
    ...
    def train_one_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        data_size_all = 0
        for data_pt in list(self.train_config.keys()):
            ...
            steps, start, end, data_size = self.prepare_data(train_data, self.args.train_batch_size)
            steps_shuffle = list(range(steps))
            shuffle(steps_shuffle)
            data_size_all += data_size
    
            for step in steps_shuffle:
                # train_data['charge'] = torch.zeros_like(train_data['atom_types'])
                # train_data['total_charge'] = torch.zeros_like(train_data['energy'])
                # train_data['dipole'] = torch.zeros_like(train_data['cell'])
                # train_data['quadrupole'] = torch.zeros(len(train_data['energy']), 5)
                batch_data = batchify(train_data, start[step], end[step], device=self.device)
                self.optimizer.step(closure)

        for k in train_rmse.keys():
            train_rmse[k] = sum(train_rmse[k]) / data_size_all
            train_mae[k] = sum(train_mae[k]) / data_size_all
