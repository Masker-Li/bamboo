...
def get_parser():
    ...
    parser.add_argument("--data_training_config", type=str, default="train_config.json", help="Path to the training config file.")
    parser.add_argument("--data_validation_config", type=str, default="val_config.json", help="Path to the validation config file.")
...
class DistillationEnsemble:
    def __init__(self, args) -> None:
        ...
        with open(os.path.join(DATA_PATH, self.args.data_training_config)) as f:
            self.train_config = json.load(f)
        with open(os.path.join(DATA_PATH, self.args.data_validation_config)) as f:
            self.val_config = json.load(f)
        self.train_path = os.path.dirname(os.path.join(DATA_PATH, self.args.data_training_config))
        self.val_path = os.path.dirname(os.path.join(DATA_PATH, self.args.data_validation_config))        
    ...
    def cluster_validation(self, model: BambooGET, val_config: Dict[str, int]) -> Dict[str, float]:
        keys = ['energy', 'forces', 'virial', 'dipole']
        val_rmse = {k: [] for k in keys}

        data_size_all = 0
        for data_pt in list(self.val_config.keys()):
            f_path = os.path.join(self.val_path, data_pt)
            if not os.path.isfile(f_path):
                continue
            d_time = time.time() - os.path.getmtime(f_path)
            if d_time < 60.0:
                continue
            val_data = torch.load(f_path, map_location='cpu')
            steps, start, end, data_size = self.prepare_data(val_data, self.args.batch_size)
            data_size_all += data_size

            for step in range(steps):
                batch_data = batchify(val_data, start[step], end[step], device=self.device)
                data_length = len(batch_data["energy"])
                mse, mae, _ = model.get_loss(batch_data)
                for k in keys:
                    val_rmse[k].append(mse[k].item() * data_length)

        for k in keys:
            val_rmse[k] = sum(val_rmse[k]) / data_size_all
        total_val_rmse = {}
        total_val_rmse["cluster_energy_rmse"] = np.sqrt(val_rmse["energy"])
        total_val_rmse["cluster_force_rmse"] =  np.sqrt(val_rmse["forces"])
        total_val_rmse["cluster_virial_rmse"] = np.sqrt(val_rmse["virial"])
        total_val_rmse["cluster_dipole_rmse"] = np.sqrt(val_rmse["dipole"])
    ...
    def run(self):
        ...
        cluster_val_rmse = self.cluster_validation(model, self.val_config)

        add_train_curve(-1, bulk_val_rmse, cluster_val_rmse)
        log_train_curve()

        # Start ensemble training
        cluster_random_train_pt = list(self.train_config.keys())
        cluster_pt_num = len(cluster_random_train_pt)
        random.shuffle(cluster_random_train_pt)
        pt_index = 0
        ...
        for epoch in range(self.epochs):
            ...
            for idx in train_order:
                ...
                for i in range(100):
                    pt_index = (pt_index + 1) % cluster_pt_num
                    data_pt = cluster_random_train_pt[pt_index]
                    f_path = os.path.join(self.train_path, data_pt)
                    train_data = torch.load(f_path, map_location='cpu')
                    steps, start, end, data_size = self.prepare_data(train_data, self.batch_size)
                    ...
                    for step in steps_shuffle:
                        ...
                        batch_data = batchify(train_data, start[step], end[step], device=self.device)
